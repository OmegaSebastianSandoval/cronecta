...
<script>
  ...
  document.addEventListener('DOMContentLoaded', () => {
    // obtiene el ID de Colombia por nombre (asegúrate que coincide con tu data)
    ...
    function initCountryStateCity({
      ...
      const cityWrapper = document.getElementById(cityWrapperId);

      countryEl.addEventListener('change', () => {
        const selected = parseInt(countryEl.value, 10);
        // reset
        ...
        // sólo si es Colombia, cargamos estados
        if (selected === colombiaId) {
          const countryObj = countriesData.find(c => c.id === selected);
      ...
      stateEl.addEventListener('change', () => {
        const selectedCountry = parseInt(countryEl.value, 10);
        const selectedState = parseInt(stateEl.value, 10);
        cityEl.innerHTML = '<option value="">Seleccione...</option>';
        cityWrapper.classList.add('d-none');

        if (selectedCountry === colombiaId && selectedState) {
          const countryObj = countriesData.find(c => c.id === selectedCountry);
          const stateObj = countryObj.states?.find(s => s.id === selectedState);
          stateObj?.cities?.forEach(city => {
            const opt = document.createElement('option');
            opt.value = city.id ?? city.name;
